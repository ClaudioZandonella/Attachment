...
\title{Evaluating Informative Hypotheses with Equality and Inequality Constraints: The Bayes Factor via the Encompassing Prior Approach}
...
To compute the Bayes Factor between different informative hypotheses, we can simply consider the ratio of the Bayes Factor of each hypothesis against the encompassing model:
\begin{equation}
...
\section{A Case Study: Hypotheses Testing in the Attachment Theory}
