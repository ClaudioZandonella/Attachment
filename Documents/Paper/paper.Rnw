...
All materials and data are available at \url{https://doi.org/10.17605/OSF.IO/JKMDF}, GitHub repository \url{https://github.com/ClaudioZandonella/Attachment}. Online supplemental material available via GitHub Pages at \url{https://claudiozandonella.github.io/Attachment/}.
...
<<settings, echo=FALSE, message=FALSE, include=FALSE>>=
library(kableExtra)
library(ggplot2)
...
knitr::opts_chunk$set(background = "#FAFAFF")
...
tar_load_paper(store = "../../Analysis/_targets/")
@
...
Only recently, \textcite{guApproximatedAdjustedFractional2018} introduced an approximate procedure to evaluate equality and inequality constraints, using the Bayes factor with the encompassing prior approach in a wide range of statistical models (i.e., GLMs, GLMMs, and SEMs). Subsequently, \textcite{mulderBayesFactorTesting2019} extended this approach to generalized multivariate probit models, and, finally, \textcite{mulderSimpleBayesianTesting2019} proposed an accurate procedure (i.e., not based on an approximation) that allows for testing of informative hypotheses with equality and inequality constraints in linear regression models.

The development and implementation of these methods are of particular interest because although they have some limits, they allow researchers to directly evaluate their expectations and hypotheses. The available literature, however, is rather technical. The complexity of these articles makes it difficult for researchers not familiar with this approach to clearly understand all the steps involved. On the other hand, articles offering a general introduction to the Bayes factor do not provide enough details to allow readers to autonomously apply this method to their problems; instead, they usually rely on ad hoc solutions implemented in some statistical software. For example, \textcite{hoijtinkTutorialTestingHypotheses2019} offered an excellent introduction to the Bayes factor for the testing hypotheses discussing all the main aspects. However, the description of the evaluation of competing informative hypotheses is limited and no specific details are provided regarding the adopted encompassing prior approach. Moreover, using ad-hoc solutions that automate the analysis, such as the functions available in the R packages \texttt{bain} \parencite{guBainBayesFactors2021} and \texttt{BFpack} \parencite{mulderBFpackFlexibleBayes2021}, prevent the users from fully understanding all the steps and limits entailed in this approach.

The aim of this paper, therefore, is to offer a clear and detailed description of the Bayes factor with the encompassing prior approach to allow other researchers to consciously apply this approach in their studies. In particular, we refer to the approximated method \textcite{guApproximatedAdjustedFractional2018} proposed, as it applies to a wider range of conditions than the more accurate approach \textcite{mulderSimpleBayesianTesting2019} introduced. In particular, researchers can use the proposed method in the case of generalized linear mixed models and structural equation models, two very general families of statistical models. Moreover, by providing an exhaustive description of all of the steps of this approach, researchers do not have to rely on already available solutions; instead, they can implement this approach according to their specific needs, aware of its strengths and limits. The paper is organized as follows. First, we introduce the method, providing a detailed description of all steps and elements involved in the formalization of informative hypotheses and Bayes factor computation. We then present an application of the method to real data to discuss the typical issues encountered in real complex scenarios.
...
Note that, prior specification is relevant only for the parameters of interest (i.e., the parameters involved in the constraints). Nuisance parameters (i.e., the parameters not involved in the constraints) are integrated out in the Bayes factor. Thus, the results will be insensitive to the actual choice of the prior for the nuisance parameters as long as it is vague enough.
...
All analyses were conducted using R statistical software \parencite[Version \Sexpr{paste(version$major,version$minor, sep = ".")};][]{rcoreteamLanguageEnvironmentStatistical2021}. All materials, data, and analysis code are available at \url{https://doi.org/10.17605/OSF.IO/JKMDF}, GitHub repository \url{https://github.com/ClaudioZandonella/Attachment}. Supplemental Material with further details is also available online via GitHub Pages at \url{https://claudiozandonella.github.io/Attachment/}.
...
To interpret these patterns, remember that the bayes factor is insensitive to the prior distribution specification for the parameters with order constraints as long as the distribution is symmetric and centered on the constraint focal point. On the contrary, in the case of equality constraints, the bayes factor is highly affected by the prior definition. As the prior gets more diffuse, the density value at zero decreases as well and, even for posterior distributions centered far from zero, the densities ratio at zero will favor the posterior. Thus, as the prior becomes more diffuse, the Bayes factor will increasingly favor the hypotheses with equality constraints. For tighter priors, the Bayes factor will strongly penalize hypotheses with equality constraints if these are not correct (see Figure~\ref{fig:plot-sensitivity-prior}).
...
Conceptualization: C. Zandonella Callegher, T. Marci, P. De Carli, and G. Altoè;
Methodology: C. Zandonella Callegher, T. Marci, P. De Carli, and G. Altoè;
Software: C. Zandonella Callegher and G. Altoè;
Formal Analysis: C. Zandonella Callegher and G. Altoè;
Investigation: T. Marci and  P. De Carli;
Data Curation: T. Marci and C. Zandonella Callegher;
Writing – Original Draft: C. Zandonella Callegher;
Writing – Review \& Editing: T. Marci, P. De Carli, and G. Altoè;
Visualization: C. Zandonella Callegher;
Supervision: G. Altoè;
Project Administration: G. Altoè;
...
\section*{Prior versions}

Preprint version of the submitted manuscript is available at \url{https://psyarxiv.com/6kc5u}.
