...
```{r}
ggplot(data_munged) +
  geom_bar(aes(x = externalizing_sum, fill = genere), alpha = 0.7) +
  facet_grid(genere ~ .)
```
...
```{r}
ggplot(data_munged) +
  geom_bar(aes(x = internalizing_sum, fill = genere), alpha = 0.7) +
  facet_grid(genere ~ .)
```
...
prop.table(table(data_cluster_tat$cluster_mother_tat))

prop.table(table(data_cluster_tat$cluster_mother_tat,
                 data_cluster_tat$genere),2) 

summary(table(data_cluster_tat$cluster_mother_tat,
              data_cluster_tat$genere))
```
...
summary(table(data_cluster_tat$cluster_father_tat,
              data_cluster_tat$genere))
```
...
Nel lavoro di @rodriguez-yamEfficientGibbsSampling, inoltre, fanno una cosa strana nel senso che definiscono $\lambda = X\beta+\epsilon$ e oltre a mettere le prior sui parametri $\beta$ la mettono anche su $\lambda$. Non capisco:
